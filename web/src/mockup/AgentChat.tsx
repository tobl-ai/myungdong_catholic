...
const demoResponses: Record<string, Message> = {
  ...
  '가까운 본당 찾아줘': {
    ...
    richCard: { type: 'parishes', data: {} },
  },
  '오늘의 성인은 누구?': {
    role: 'ai',
    content: '오늘(3월 3일)의 성인은 **성녀 쿠네군다**(Cunegunda)입니다.\n\n신성 로마 제국 황후로, 남편 성 하인리히 2세와 함께 수많은 성당과 수도원을 건립. 남편 사후 수도원에 입회하여 여생을 기도와 봉사로 보냈습니다.\n\n**한 줄 묵상**: 높은 지위에서도 겸손과 봉사를 실천한 삶.',
...
export function AgentChat({ initialQuery, onClose, isDesktopPanel }: Props) {
  ...
  return (
    <div className={`flex flex-col bg-[#fafaf8]
      ${isDesktopPanel ? 'h-full' : 'h-[calc(100vh-7.5rem)]'}`}>
      {/* Header */}
      <div className="px-5 py-3 flex items-center gap-3 border-b border-black/5 shrink-0">
        <button onClick={onClose} className="md:hidden text-black/30 hover:text-black/60 transition-colors text-sm">←</button>
        <div className="flex-1">
          <span className="text-xs font-medium">✦ AI 상담</span>
          <span className="text-[10px] text-black/25 ml-2">27,000건 교회 문헌 기반</span>
        </div>
        <button
          onClick={() => { setMessages([]); prevQuery.current = '' }}
      ...
      <div className="flex-1 overflow-y-auto px-5 space-y-5 py-4">
        {messages.length === 0 && !typing && (
          <div className="text-center py-16">
            <div className="w-12 h-12 mx-auto rounded-2xl bg-[#1a1a1a] flex items-center justify-center mb-4">
              ...
              <div className="max-w-[95%]">
                ...
                {msg.richCard?.type === 'parish' && (
                  <div className="mt-3 bg-white rounded-xl border border-black/5 p-4">
                    ...
                    <button className="mt-3 w-full py-2 bg-[#1a1a1a] text-white text-xs rounded-lg">길찾기 →</button>
                  </div>
                )}
                ...
                {msg.sources && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {msg.sources.map((s, k) => (
                      <span key={k} className="text-[10px] text-black/25 bg-black/[0.03] px-2 py-0.5 rounded-md">{s}</span>
                    ))}
                  </div>
                )}
              </div>
        ...
        {typing && (
          <div className="flex items-center gap-1.5 py-2">
            {[0, 1, 2].map(i => (
              <div key={i} className="w-1.5 h-1.5 rounded-full bg-black/20 animate-bounce" style={{ animationDelay: `${i * 150}ms` }} />
            ))}
          </div>
        )}
        <div ref={endRef} />
      ...
      {/* Input */}
      <div className="px-5 pb-3 md:pb-4 pt-2 shrink-0">
